...
class BallsPhysics : public olc::PixelGameEngine
{
public:
	BallsPhysics()
	{
...
public:
	...
	float fBallTimer = 0, fBallFrequency = 0;
	bool bBounce = false, bStability = false, bGravity = false;
...
public:
	...
	void AddBall(vf2d pos, float r = 5.0f)
	{
		sBall b;
		b.p = pos;
		b.v = { 0, 0};
...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		if (GetMouse(1).bPressed && smCue != PositionCue)									// Add Ball by mouse click
			AddBall({ (float)GetMouseX(), (float)GetMouseY() }, rand() % 16 + 2);
		...
		DrawStringDecal({ 4,4 }, std::to_string(fBallFrequency) + " bps");
		DrawStringDecal({ 4,12 }, bStability ? "Stable" : "Unstable");
		DrawStringDecal({ 4,20 }, bGravity ? "Gravity ON" : "Gravity OFF");
	}
};
